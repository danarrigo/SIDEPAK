...
export default function LeaderboardClient({ memberId, koperasiData, provinsiData, nasionalData }: LeaderboardClientProps) {
  ...
  return (
    <div className="glass-card border border-outline-variant rounded-2xl p-6 flex flex-col h-[520px]">
      ...
      <div className="flex bg-surface-container-low rounded-xl p-1 mb-6 border border-outline-variant/30 shrink-0">
        <button
          ...
          <>
            ...
            {data.length > 6 && (
              <div className="flex justify-center py-1">
                <span className="material-symbols-outlined text-on-surface-variant/50">more_horiz</span>
              </div>
            )}

            {data.length > 4 && data.slice(Math.max(4, data.length - 2)).map((m, relativeIdx) => {
              const actualIdx = Math.max(4, data.length - 2) + relativeIdx;
              return (
                <div key={m.id} className={`flex items-center justify-between p-3 rounded-xl border ${m.id === memberId ? 'border-tertiary bg-tertiary/10' : 'border-outline-variant/30 bg-surface-container'}`}>
                  <div className="flex items-center gap-4">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs bg-surface-variant text-on-surface-variant`}>
                      {actualIdx + 1}
                    </div>
                    <div>
                      <p className="text-sm font-bold text-on-surface">{m.namaLengkap.split(' ')[0]}</p>
