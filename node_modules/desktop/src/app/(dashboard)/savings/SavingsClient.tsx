...
import {
  payDuesFromWallet,
  depositSavingsFromWallet,
  withdrawSavingsToWallet,
...
export default function SavingsClient({
  ...
  const totalSimpanan =
    financials.simpananPokok +
    financials.simpananWajib +
    financials.simpananSukarela;
  ...
  const handlePayDues = async (type: "initial" | "monthly") => {
    ...
    if (res.success) {
      showMessage(
        `Pembayaran Simpanan ${type === "initial" ? "Pokok" : "Wajib"} berhasil!`,
        "success",
      );
      router.refresh();
    } else {
  ...
  const handleDeposit = async () => {
    ...
    const res = await depositSavingsFromWallet(
      memberId,
      amount,
      "Simpanan Sukarela Mandiri",
    );
    if (res.success) {
  ...
  const handleWithdrawToWallet = async () => {
    const amount = parseInt(depositAmount, 10);
    if (isNaN(amount) || amount <= 0) {
      showMessage("Masukkan jumlah penarikan yang valid.", "error");
      return;
    }

    setLoadingType("withdraw_to_wallet");
    const res = await withdrawSavingsToWallet(
      memberId,
      amount,
      "Tarik Simpanan Sukarela ke Dompet",
    );
    if (res.success) {
      showMessage("Berhasil tarik ke Dompet!", "success");
      setDepositAmount("");
      router.refresh();
    } else {
      showMessage(res.error || "Terjadi kesalahan.", "error");
  ...
  for (const w of financials.disbursements || []) {
    combinedTxs.push({
      type: "disbursement",
  ...
  return (
    <main className="flex-1 flex flex-col min-h-screen bg-background pb-24 md:pb-0">
      <div className="flex-1 overflow-y-auto px-6 py-10 space-y-8 w-full max-w-5xl mx-auto">
          ...
          <div>
            <div className="flex items-center gap-4 mb-2">
              ...
              <h1 className="text-2xl font-black text-on-surface">
                Kelola Simpanan & Mutasi
              </h1>
            </div>
            <p className="text-sm text-on-surface-variant ml-12">
              Pantau dan kelola kewajiban simpanan Koperasi Anda dari satu
              tempat.
            </p>
          </div>

          <div className="flex items-center gap-6 bg-surface-container-highest px-6 py-4 border border-outline-variant/30 rounded-2xl ml-12 md:ml-0 shadow-sm">
            ...
            <Link
              href="/withdraw"
              className="px-4 py-2.5 rounded-xl bg-primary text-on-primary font-bold text-xs hover:bg-primary/90 transition-colors flex items-center gap-1.5 shadow-sm"
            >
        ...
        {message && (
          <div
            className={`p-4 rounded-xl border animate-fade-in ${message.type === "success" ? "bg-primary/10 border-primary/30 text-primary" : "bg-error/10 border-error/30 text-error"}`}
          >
        ...
        <div className="glass-card animate-slide-up delay-100 border border-outline-variant rounded-2xl p-8 flex flex-col items-center justify-center text-center bg-gradient-to-br from-primary/5 to-transparent">
          <p className="text-xs uppercase font-extrabold tracking-widest text-on-surface-variant mb-2">
            TOTAL SIMPANAN ANDA
          </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-slide-up delay-200">
          ...
          <div className="glass-card border border-outline-variant rounded-2xl p-6 flex flex-col justify-between">
            <div>
              <div className="flex justify-between items-start mb-4">
                <div>
                  ...
                  <p className="text-xs text-on-surface-variant mt-1">
                    Dibayar 1x saat bergabung
                  </p>
                </div>
                <span
                  className={`text-[10px] font-extrabold px-3 py-1 rounded-full border ${financials.isPokokPaid ? "bg-primary/10 text-primary border-primary/20" : "bg-error/10 text-error border-error/20"}`}
                >
              ...
              <div className="bg-surface-container-low p-4 rounded-xl border border-outline-variant/30 mb-6">
                ...
                <p className="text-xl font-black text-on-surface mt-1">
                  Rp {financials.simpananPokok.toLocaleString("id-ID")}
                </p>
              </div>
            </div>
          ...
          <div className="glass-card border border-outline-variant rounded-2xl p-6 flex flex-col justify-between">
            <div>
              <div className="flex justify-between items-start mb-4">
                ...
                <span
                  className={`text-[10px] font-extrabold px-3 py-1 rounded-full border ${financials.isWajibPaidThisMonth ? "bg-primary/10 text-primary border-primary/20" : "bg-error/10 text-error border-error/20"}`}
                >
              ...
              <div className="bg-surface-container-low p-4 rounded-xl border border-outline-variant/30 mb-6">
                ...
                <p className="text-xl font-black text-on-surface mt-1">
                  Rp {financials.simpananWajib.toLocaleString("id-ID")}
                </p>
              </div>
            </div>
          ...
          <div className="glass-card border border-primary/30 rounded-2xl p-6 flex flex-col justify-between relative overflow-hidden shadow-[0_0_15px_rgba(22,163,74,0.1)]">
            ...
            <div>
              <div className="flex justify-between items-start mb-4">
                <div>
                  <h3 className="font-bold text-on-surface text-lg text-primary">
                    Simpanan Sukarela
                  </h3>
                  <p className="text-xs text-on-surface-variant mt-1">
                    Bebas menabung kapan saja
                  </p>
                </div>
                <span className="material-symbols-outlined text-primary text-3xl">
                  savings
              ...
              <div className="bg-surface-container-low p-4 rounded-xl border border-outline-variant/30 mb-4">
                ...
                <p className="text-xl font-black text-on-surface mt-1">
                  Rp {financials.simpananSukarela.toLocaleString("id-ID")}
                </p>
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="flex gap-2 w-full">
                <button
                  ...
                  className="flex-1 py-3 rounded-xl bg-primary text-on-primary font-bold text-sm hover:bg-primary/90 transition-colors disabled:opacity-70 flex justify-center items-center gap-2"
                >
                  {loadingType === "deposit" ? (
                    <span className="material-symbols-outlined animate-spin text-sm">
                      sync
                ...
                <button
                  ...
                  disabled={loadingType === "withdraw_to_wallet" || !depositAmount}
                  className="flex-1 py-3 rounded-xl bg-secondary text-on-secondary font-bold text-sm hover:bg-secondary/90 transition-colors disabled:opacity-70 flex justify-center items-center gap-2"
                >
                  {loadingType === "withdraw_to_wallet" ? (
                    <span className="material-symbols-outlined animate-spin text-sm">
                      sync
                    </span>
                  ) : null}
        ...
        <div className="glass-card animate-slide-up delay-300 border border-outline-variant rounded-2xl p-6">
          <h3 className="font-bold text-on-surface text-lg mb-6">
          ...
          {combinedTxs.length === 0 ? (
            ...
            <div className="space-y-4">
              {combinedTxs.map((tx, idx) => (
                <div
                  key={idx}
                  className="flex items-center justify-between p-4 rounded-xl bg-surface-container-low border border-outline-variant/30 hover:border-outline transition-colors"
                >
                  <div className="flex items-center gap-4">
                    <div
                      className={`w-10 h-10 rounded-full flex items-center justify-center ${tx.isPositive ? "bg-primary/10 text-primary" : "bg-error/10 text-error"}`}
                    >
                    ...
                    <div>
                      ...
                      <p className="text-xs text-on-surface-variant mt-1">
                        {tx.rawDate.toLocaleDateString("id-ID", {
                          day: "numeric",
                          month: "long",
                          year: "numeric",
                        })}{" "}
                        • {tx.type === "dues" ? "Iuran" : tx.type === "disbursement" ? "Penarikan" : "Simpanan"}
                      </p>
                    </div>
                  </div>
                  <div
                    className={`font-black ${tx.isPositive ? "text-primary" : "text-error"}`}
                  >
                    {tx.isPositive ? "+" : "-"} Rp{" "}
